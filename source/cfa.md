...
* To provide a means of storing metadata that relate to each fragment, but which are not necessary for the creation of the aggregated data.
  ...
  For particular, it may be convenient for a metadata property that is defined within each fragment file to be made available to the aggregated variable, without having to open and inspect the fragment files themselves.
