...
* An important part of the CFA conventions is the ability to mix,
  within the same CFA-netCDF file, normal CF-netCDF variables (with
  possibly very large data arrays) with CFA variables (which may
  contain references to CFA private variables with possibly very large
  data arrays in the same file), so a binary format is desirable to
...
A simple CFA-netCDF file::
   ...
   variable: 
       ...
       float tas ; 
               ...
               tas:cfa_array = "{'directions': {'lat': false,
                                                'time': true,
                                                'lon': true
                                               },
                                 'pmshape': [2],
